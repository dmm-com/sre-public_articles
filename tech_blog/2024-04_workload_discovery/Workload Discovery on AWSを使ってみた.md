...
Workload Discovery on AWS とは AWSクラウドのワークロードを可視化するソリューションです。
...
本記事では本ソリューションをセットアップする具体的な手順には触れず、本ソリューションが提供する主要な機能の1つであるシステム構成図の作成に焦点を当てて説明します。

# 実践的な使用例
Workload Discovery on AWS にはいくつか機能がありますが、本記事ではシステム構成図の作成に焦点を当てて説明します。

## AWSのシステム構成図作成
Workload Discovery on AWS を利用して AWS のシステム構成図を作成する手順は以下の通りです。
...
# まとめ
Workload Discovery on AWS を使うことで、いつでも短時間で最新のシステム構成図を作成できるようになります。

これにより、保守／開発しているメンバが最新のシステム構成について、より深く理解できるようになることが期待できます。

この記事を通じて、本ソリューションがどのようにシステム構成図の作成やシステム構成の把握に効果的であるかを理解していただけたと思います。
