...
本記事では、コスト削減を目的とした、AWS Elastic Container Registry (以下、ECR)の
機能の１つである、プルスルーキャッシュリポジトリ機能の使い方についてご紹介していきます。
...
# プルスルーキャッシュリポジトリ利用時の全体イメージ

プルスルーキャッシュリポジトリを利用した際の全体イメージは以下のとおりです。

1. 初回は、NAT経由 で ECR Public Registry からイメージ取得
...
※ <region> は実際に使いたいリージョン名として読み替えてください
...
アップストリームレジストリとしては、Docker Hub や GitHub コンテナレジストリなど、
...
2. `キャッシュ名前空間` の `キャッシュリポジトリプレフィックス` で任意のネームスペースを指定してください。
   ...
   - もし、同一AWSアカウント内で複数システムが稼働してるなど、分ける必要があれば、それに応じてネームスペースを設定してください。
...
1. IAMユーザー/ロールへのポリシー設定
  - 他のIAM権限と同様に、ユーザーまたはロール単位で権限を設定します。
2.  ECRプライベートレジストリへのポリシー設定
  - リポジトリ単位での権限設定となります。
...
※ IAMポリシーの Resouce に対応する項目なので、ワイルドカードが指定できますが、
　GUI上ではパス内にアスタリスクを含めると、入力チェックエラーでうまく設定できないようです。
　ワイルドカードを含めたい場合は、JSON形式であれば編集、保存が可能でした。
　（2025/1/24 時点）
